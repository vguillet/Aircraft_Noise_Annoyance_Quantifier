...
Plot_oaspl_ospl = false;        %Plot results of the A_weighted module

Plot_r = false;                 %Plot r versus time

Plot_Uncorrected = false;       %Plot a spectogram of the data uncorrected (experimental)
...
%Advanced options
%-----------------------------------------------------
%Change to false to switch off modules in the code
A_weighted = false;
Tonality = false;
Metrics = true;     %(epnl,aspl,Loudness)

Sharpness = true;   %(for copatibility only, do not set to true if Metrics module is set to false)
...
manual_R = 0;          %Roughness
manual_F = 0;          %Frequency
...
S = 0;                   %preset for S in case the sharpness module is switched off
Tonal = 0;               %preset for Tonal in case the tonality module is switched off
phonav = 0;              %preset for phonav in case the metrics module is switched off
...
if reference_nb == 0
    ...
    XXX = ['Dataset reference_nb selected: ', num2str(reference_nb)];
    disp(XXX)
...
%aspl calculation
    m2 = @asplcalc;
    aspl = m2(power,frequency,index);

%Loudness metric:
    m3 = @loudness;
    [phonresult,phonav] = m3(aspl);
end
...
m5 = @psyA;
[PA,PAt] = m5(reference_nb,Tonal,S,phonav,manual_R,manual_F);
...
if A_weighted == false
    disp("A_weighted module switched off")
end

if Tonality == false
    disp("Tonality module switched off")
end

if Metrics == false
    disp("Metrics module switched off")
end

if Sharpness == false
    disp("Sharpness module switched off")
end
...
if Tonality == true
    XXX = ['Tonality: ', num2str(Tonal)];
    disp(XXX)
end

if Metrics == true
    XXX = ['Effective perceived noise level: ', num2str(epnl_value)];
    disp(XXX)
    
    %XXX = ['Average sound pressure level: ', num2str(aspl)];
    %disp(XXX)
    
    XXX = ['Loudness: ', num2str(phonav)];
    disp(XXX)  
end

if Sharpness == true
    XXX = ['Sharpness: ', num2str(S)];
    disp(XXX)
end

if Metrics == false
    disp('Metrics module needs to be switched on for Psychoacoustic annoyance to be computed')
else
    XXX = ['Psychoacoustic annoyance EXCLUDING tonality: ', num2str(PA)];
    disp(XXX)
    
    if Tonality == false
        disp('Tonality module needs to be switched on for Psychoacoustic annoyance INCLUDING tonality to be computed')
    else
        XXX = ['Psychoacoustic annoyance INCLUDING tonality: ', num2str(PAt)];
        disp(XXX)
    end
end
...
for h = 1:size(time,2)
    ...
    for n=1:length(freqpowsorted)
        if isnan(freqpowsorted(n,2)) == 1
            freqpowsorted(n,2)=0;
        else
            continue
        end
    ...
    avspl = zeros(24,1);
    plist = zeros(24,(length(lowfreq)-1)); 
    ...
    for k = 1:(length(lowfreq)-1)
        ...
        p = 0;
        for t = 1:length(fps)
            if fps(t,1)>lowfreq(k,1) && fps(t,1)<lowfreq((k+1),1)
                powerr = 10^(fps(t,2)/10);
                p = p + sqrt(powerr);
                plist(t,k) = p;
            end
        end
...
%Loudness metric:
function [phonresult,phonav] = loudness(aspl)
...
%psychoacoustic annoyance
function [PA,PAt] = psyA(reference_nb,Tonal,S,phonav,manual_R,manual_F)

Rlst = [2.78,2.28,3.69,2.94,3.19,3.13,2.4,2.35,3.13,2.58,2.33,2.63,2.93];   %Roughness
Flst = [1.853,1.773,1.313,1.57,1.582,2.,1.319,1.817,1.25,1.66,1.27,1.43,1.38];   %Fluctuation strength
...
PA = phonav*(1+sqrt((0.25*log(phonav+10)*(S-1.75))^2+(2.18/(phonav^0.4)*(0.4*F+0.6*R))^2));
