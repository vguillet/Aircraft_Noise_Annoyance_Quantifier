...
if reference_nb == 0
    ...
    h = [60.44,58.79,67.45,59.74,58.01,53.20,57.92];
    h0 = h(reference_nb);
    %velocity matching aircraft data set [m/s]
    v = [68.46,66.82,86.73,71.97,58.90,62.45,57.97];            
    v0 = v(reference_nb);
end
...
for i=1:length(phonresult)
    z_value=13*atan(0.76*phonresult(i,2)/1000)+3.5*atan(phonresult(i,2)/7500).^2;
